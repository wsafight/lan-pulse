...
- Android UDP/RTP 接收和低延迟 `AudioTrack` PCM 播放，包含 15 ms 初始自适应缓冲、10-60 ms 调整范围、丢包静音补偿、时钟漂移修正、流超时检测和 3 秒重连。
...
- 桌面媒体发送任务带有界退避自愈、捕获丢帧/重启诊断和显式捕获线程回收。
...
- 可信设备持久化、MediaSession、详细抖动诊断和长时间真机调优尚未实现。
